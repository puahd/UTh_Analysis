...
def willFilesBeMoved(path):
    ...
    if len(glob.glob(os.path.join('*.exp')) + glob.glob(os.path.join('*blk*.exp')) + glob.glob(os.path.join('*yhasU*.exp')) + glob.glob(
            os.path.join('*yhasTh*.exp')) + glob.glob(os.path.join('*hf*.exp'))) > 0:
        os.chdir(old_path)
        return True
    else:
        os.chdir(old_path)
...
def willFilesBeDeleted(path):
    ...
    if len(glob.glob(os.path.join('*.TDT')) + glob.glob(os.path.join('*.dat')) + glob.glob(os.path.join('*.log')) + glob.glob(os.path.join('*.ini')) + glob.glob(
            os.path.join('*.TXT'))) > 0:
        os.chdir(old_path)
        return True
    else:
...
def createDataFolders(path):
    ...
    # blanks
    blanks = glob.glob(os.path.join('*blk*.exp'))
    for b in blanks:
        shutil.move(b, 'blank')
    # yhas_u
    yhasu = glob.glob(os.path.join('*yhasU*.exp'))
    for u in yhasu:
        shutil.move(u, 'yhas_u')
    # yhas_th
    yhasth = glob.glob(os.path.join('*yhasTh*.exp'))
    for t in yhasth:
        shutil.move(t, 'yhas_th')
    # hf
    hf = glob.glob(os.path.join('*hf*.exp'))
    for h in hf:
        shutil.move(h, 'hf')
    # data
    datar = glob.glob(os.path.join('*.exp'))
    for d in datar:
        shutil.move(d, 'data')
...
def removeUnnecessaryFiles(path):
    old_path = os.getcwd()
    os.chdir(path)
    files = glob.glob(os.path.join('*.TDT')) + glob.glob(os.path.join('*.dat')) + glob.glob(os.path.join('*.log')) + glob.glob(os.path.join('*.ini')) + glob.glob(
        os.path.join('*.TXT'))
