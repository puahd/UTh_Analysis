...
class Analyzer:
    ...
    def set_metadata(self, metadata_path, ratios):
        ...
        if metadata_path.endswith('.csv'):
            ...
            possible_names = ['Ergebnisse', 'Results']
            for sheet in possible_names:
                if sheet in pd.ExcelFile(metadata_path).sheet_names:
                    fullMetadata = pd.read_excel(metadata_path, sheet_name= sheet, na_filter=False, engine='openpyxl')
    ...
    def analyze(self, ratios, filename='Results', write_results_file=True, options_dict=None, output_path=None):
        ...
        if len(ratios.index) + 1 == len(self.metadata.index):
            ...
            ratios = pd.concat([ratios, ratios.iloc[-2].to_frame().T], ignore_index=False)
    ...
    def writeToFile(self, results_dict, fileTitle='Results', output_path=None):
        writer = ExcelWriter(os.path.join(self.data_root_folder, '{}.xlsx'.format(fileTitle)), engine='xlsxwriter')
