...
from PyQt5.QtWidgets import QWidget, QGridLayout, QGroupBox, QLineEdit, QLabel, QPushButton, QFileIconProvider, \
    ...
    QTableView, QStyle, QAbstractItemView
...
class InputTabWidget(QWidget):
    ...
    def runEvent(self):
        ...
        if not os.path.isdir(path):
            ...
            if (DataFolderUtil.willFilesBeMoved(path) and not DataFolderUtil.willFilesBeDeleted(path) and QMessageBox.question(
                ...
                self.window.inspectTab.set_path(path)
    ...
    def openOutputDirectory(self):
        ...
        if platform == 'win32':
            os.startfile(path)  # Opens the folder in File Explorer
        elif platform == 'darwin':  # macOS
            subprocess.call(('open', path))  # Opens the folder in Finder
        else:  # Linux (or other Unix-like systems)
            subprocess.call(('xdg-open', path))  # Opens the folder in the default file manager
    ...
    def initOverviewBox(self):
        ...
        self.uTailTable.setVerticalScrollMode(QAbstractItemView.ScrollPerPixel)
        self.uTailTable.setHorizontalScrollMode(QAbstractItemView.ScrollPerPixel)

        #uThTailTable
        self.thTailTable = QTableView()
        self.thTailTable.verticalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.thTailTable.setVerticalScrollMode(QAbstractItemView.ScrollPerPixel)
        self.thTailTable.setHorizontalScrollMode(QAbstractItemView.ScrollPerPixel)

        #hPlusTable
        self.factorsTable = QTableView()

        self.factorsTable.verticalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.factorsTable.setVerticalScrollMode(QAbstractItemView.ScrollPerPixel)
        self.factorsTable.setHorizontalScrollMode(QAbstractItemView.ScrollPerPixel)
    ...
    def initRatiosBox(self):
        ...
        self.ratiosTable.setVerticalScrollMode(QAbstractItemView.ScrollPerPixel)
        self.ratiosTable.setHorizontalScrollMode(QAbstractItemView.ScrollPerPixel)
