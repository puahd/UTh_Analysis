...
def get_ratios_and_metadata_from_results(file_path):
    if os.path.isfile(file_path):
        ...
        metadata = metadata[['Lab. #', 'Denomination', 'Sample type', 'Meas. Dat.', 'Depth',
...
def openFileItem(path, item):
    ...
    if os.path.isfile(itemPath):
        if platform == 'win32':
            os.startfile(itemPath)  # Opens the file with the default application (Notepad for .txt files)
        elif platform == 'darwin':  # macOS
            subprocess.call(('open', itemPath))  # Open the file with the default text editor (TextEdit)
        else:  # Linux
            subprocess.call(('xdg-open', itemPath))
        # if platform == 'win32':
        #     os.system("notepad.exe {}".format(itemPath))
        # elif platform == 'darwin':
        #     os.system("open -a TextEdit {}".format(itemPath))
